...
int main(int argc, char const *argv[])
{
    ...
    if (argc < 2)
    {
        ...
        cout << "Algoritmos disponibles: all, heuristico, ls, ils, tabu, sa, grasp, ag, memetico, aco, ss, cro" << endl;
        return 1;
    }
    ...
    bool runCRO = runAll || (algoritmo.find("cro") != string::npos);

    bool necesitaHeuristica = runH || runLS || runILS || runTS || runSA || runAG || runAM || runSS || runACO || runCRO;
    ...
    if (archivoCSVCabecera.is_open())
    {
        ...
        if (runCRO)
            archivoCSVCabecera << ",Costo CRO,T. CRO(s)";
        archivoCSVCabecera << "\n";
...
#pragma omp parallel for schedule(dynamic)
    for (int f = startFileIdx; f < argc; f++)
    {
        ...
        vector<double> tH, tLS, tILS, tTS, tSA, tGRASP, tAG, tAM, tSS, tACO, tCRO;
        vector<double> cH, cLS, cILS, cTS, cSA, cGRASP, cAG, cAM, cSS, cACO, cCRO;
        ...
        for (int iter = 0; iter < numCorridas; iter++)
        {
            ...
            if (runCRO)
            {
                vector<TBool> varsParaCRO = varsInicial;
                auto start = chrono::high_resolution_clock::now();
                problema.optimizacionReaccionQuimica(varsParaCRO, 20, 2.0, gen);
                auto end = chrono::high_resolution_clock::now();
                chrono::duration<double> diff = end - start;
                cCRO.push_back(problema.calcularCosto(varsParaCRO));
                tCRO.push_back(diff.count());
            }
        }
...
#pragma omp critical
        {
            ...
            if (archivoCSVSync.is_open())
            {
                ...
                if (runCRO)
                    archivoCSVSync << "," << (int)round(promedio(cCRO)) << "," << formatearMedida(promedio(tCRO), desviacionEstandar(tCRO, promedio(tCRO)));
                archivoCSVSync << "\n";
